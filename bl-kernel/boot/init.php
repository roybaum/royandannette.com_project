...
// Environment flags based on request host.
$httpHostRaw = isset($_SERVER['HTTP_HOST']) ? strtolower(trim($_SERVER['HTTP_HOST'])) : '';
$httpHost = parse_url('http://' . $httpHostRaw, PHP_URL_HOST);
if ($httpHost === false || $httpHost === null) {
	$httpHost = $httpHostRaw;
...
define('BLUDIT_IS_LOCAL', ($isLocalHost || $isTestTLD));
define('BLUDIT_IS_PRODUCTION', !BLUDIT_IS_LOCAL);
define('BLUDIT_ENV', BLUDIT_IS_LOCAL ? 'local' : 'production');
